...
- [ ] The OpenSpec change carries a `handover` delta that records the annotation and amends the Markdown document shape, rather than the living spec being hand-edited. — check: `sh -c 'grep -rq "→ runs" openspec/changes/*/specs/handover/spec.md'` → exits 0
...
- Write the `handover` spec delta under openspec/changes/<change-id>/specs/handover/spec.md as a `## MODIFIED Requirements` block covering the Markdown document shape (:117-134) and the transfer-handover scenario (:140-143). — openspec/specs/ is archive output in this repo, not a hand-edited file — the prior change shipped its handover edit the same way.
