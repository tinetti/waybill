...
**Supersedes**: contract-2026-09-10.html
...
- [ ] The help card walkthrough names the session route and keeps the shell cd — check: `grep -Fq "/waybill:next <branch>/<leg>" tests/golden/help.txt && grep -Fq "cd <the path bay printed>" tests/golden/help.txt` → exits 0 (the first half fails today)
...
- src/help.js FROM ZERO walkthrough (steps 3 and 5) names the session route `/waybill:next <branch>/<leg>` beside the shell `cd`; the card line count is unchanged; tests/golden/help.txt re-minted — /waybill:help landed on main (#12) after approval; without this the reference card contradicts the handover
...
- **Fold the /waybill:help walkthrough update into this effort** — rejected: A separate follow-up after merge (the pre-change plan left help out of scope while it was unmerged). #12 landed after approval; shipping without it leaves the reference card contradicting the handover for a release
