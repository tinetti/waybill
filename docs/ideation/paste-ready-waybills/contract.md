...
- [ ] Markdown-mode tests exist and run. — check: `node --test --test-reporter=spec --test-name-pattern=markdown tests/ 2>&1 | grep -qE '^ +✔ .*markdown'` → exits 0 (a test whose name contains 'markdown' ran and passed)
...
- [ ] No golden carries the `└ model · effort` caption. — check: `test -z "$(cat tests/golden/*.txt tests/golden/*.md 2>/dev/null | grep '└ ')"` → exits 0 (no golden line carries the caption)
...
- [ ] `--json` with `--markdown` is rejected with exit 2 and an error naming the conflict. — check: `node src/cli.js next --json --markdown >/dev/null 2>&1; [ $? -eq 2 ] && node src/cli.js next --json --markdown 2>&1 | grep -q 'cannot be combined'` → exits 0
...
- [ ] `waybill bay --markdown` puts its `cd` line in a fence of its own. — check: `node --test --test-reporter=spec --test-name-pattern='bay --markdown' tests/ 2>&1 | grep -qE '✔ .*bay --markdown'` → exits 0 (the bay --markdown CLI test ran and passed)
...
- **Name-pattern checks grep for a passing test line naming the pattern; the caption check tolerates a missing glob; the flag-conflict check matches the spec's 'cannot be combined' text.** — rejected: `grep 'pass [1-9]'`, `! grep -l` over an unmatched glob, and grepping for `--markdown` in the error (the pre-verify checks).. A pre-implementation verify run passed all four vacuously: Node's runner reports 'pass 1' for the file even when no test matches, a missing *.md glob makes grep error and `!` flips it, and today's unknown-option error already names `--markdown`.
